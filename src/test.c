...
void run_tests() {
    ...
    printf("forward: \n");
    matrix matmean = mean(mat, 4);
    print_matrix(matmean);
    matrix matvar = variance(mat, matmean, 4);
    print_matrix(matvar);
    print_matrix(normalize(mat, matmean, matvar, 4));

    printf("backward: \n");
    matrix d = copy_matrix(mat);
    print_matrix(d);
    matrix dm = delta_mean(d, matvar, 4);
    print_matrix(dm);
    matrix dv = delta_variance(d, mat, matmean, matvar, 4);
    print_matrix(dv);
    print_matrix(delta_batch_norm(d, dm, dv, matmean, matvar, mat, 4));
}
